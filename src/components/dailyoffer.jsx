...
function DailyOffers (props) {
  const [daliyoffers , setDailyoffers] = useState([]);
  ...
  const getDailyoffers = () => {
    axios 
      .get('https://test.yarnroad.com/api/todaylist').then((response) => {setDailyoffers(response.data);
        })
      .catch((error) => {
        console.log(error);
      });
